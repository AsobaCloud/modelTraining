#!/usr/bin/env python3
"""
Merge LoRA adapters with base models and push to Hugging Face Hub
"""
...
def merge_lora_with_base(base_model_name, lora_path, output_path, model_type="qwen"):
    """Merge LoRA adapters with base model"""
    print(f"Loading base model: {base_model_name}")
    
    # Load base model
    if model_type == "qwen":
        # For Qwen, we need to handle the large model carefully
...
def create_model_card(local_path, model_name, model_type, base_model, dataset_info):
...
datasets:
- custom
model_type: {"qwen" if "qwen" in model_type.lower() else "mistral"}
---
...
- **Model Size**: {"~28GB" if "qwen" in model_type.lower() else "~14GB"}
...
- **Minimum VRAM**: {"~16GB for inference" if "qwen" in model_type.lower() else "~8GB for inference"}
- **Recommended**: {"24GB+ VRAM for smooth operation" if "qwen" in model_type.lower() else "16GB+ VRAM for smooth operation"}
...
def main():
    ...
    parser.add_argument("--skip-merge", action="store_true", 
                        help="Skip merging and use existing merged models")
    ...
    if not token and not args.skip_hf_upload:
    ...
    if args.model in ["qwen", "both"]:
        models.append({
            "name": "qwen",
            "base_model": "Qwen/Qwen2.5-Coder-14B",  # Using Qwen2.5 as base
    ...
    for model_config in models:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not args.skip_merge:
                ...
                print("\n1. Downloading LoRA model from S3...")
                ...
                if not args.skip_s3_upload:
                    print(f"\n3. Uploading merged model to S3...")
                ...
                # Download already merged model from S3
                print("\n1. Downloading merged model from S3...")
                download_from_s3(model_config["merged_s3_path"], str(merged_path))
