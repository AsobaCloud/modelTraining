...
logger = logging.getLogger("smart_security_compliance")

class SmartSecurityComplianceCollector(SmartCollectorBase):
    """Smart collector for code generation examples using content-first approach"""
    
    def __init__(self, github_token: Optional[str] = None):
        super().__init__("security_compliance", github_token)
        ...
        self.search_config = {
            'vulnerability_scanning': {
                ...
                'content_patterns': [
                    'scan', 'vulnerability', 'security', 'audit', 'check',
                    'bandit', 'semgrep', 'sonar', 'snyk', 'report'
                ],
                'min_stars': 200,
                'max_repos': 25,
                'target_examples': 100
            ...
            'security_policies': {
                ...
                'content_patterns': [
                    'policy', 'rule', 'constraint', 'validate', 'enforce',
                    'falco', 'gatekeeper', 'admission', 'security'
                ],
                'min_stars': 200,
                'max_repos': 25,
                'target_examples': 100
            ...
            'secrets_management': {
                ...
                'content_patterns': [
                    'secret', 'vault', 'encrypt', 'decrypt', 'key',
                    'credential', 'password', 'token', 'auth'
                ],
                'min_stars': 300,
                'max_repos': 25,
                'target_examples': 100
            ...
            'compliance_frameworks': {
                ...
                'content_patterns': [
                    'compliance', 'audit', 'gdpr', 'hipaa', 'framework',
                    'regulation', 'standard', 'check', 'validate'
                ],
                'min_stars': 200,
                'max_repos': 25,
                'target_examples': 100
            ...
            'security_automation': {
                ...
                'content_patterns': [
                    'automation', 'monitor', 'alert', 'incident', 'response',
                    'siem', 'soar', 'security', 'threat', 'detection'
                ],
                'min_stars': 200,
                'max_repos': 30,
                'target_examples': 100
...
def main():
    ...
    collector = SmartSecurityComplianceCollector()
    
    # Collect examples
    examples = collector.collect_domain_examples(target_count=500)
    
    # Save to file
    output_path = "/home/shingai/sort/deployments/data/corpus/smart_security_compliance_corpus.jsonl"
