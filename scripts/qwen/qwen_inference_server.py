#!/usr/bin/env python3
"""
Qwen Inference Server - FastAPI server for Qwen3-14B with CLAUDE.md methodology
"""
...
logger = logging.getLogger("qwen_inference_server")
...
async def load_model():
    """Load the Qwen model with 8-bit quantization"""
...
app = FastAPI(
    title="Qwen CLAUDE.md Inference Server",
    description="FastAPI server for Qwen3-14B with integrated CLAUDE.md methodology",
    version="1.0.0",
    lifespan=lifespan
)
...
async def generate_code(request: GenerationRequest):
    """Generate code using Qwen with CLAUDE.md methodology"""
    ...
    try:
        ...
        if complexity == "simple":
            max_new_tokens = 50  # Short response for simple questions
        elif complexity == "medium":
            max_new_tokens = 200  # Medium response for planning tasks
        else:  # complex
            max_new_tokens = 500  # Longer response for complex methodology
...
async def root():
    """Root endpoint"""
    return {
        "message": "Qwen CLAUDE.md Inference Server",
        "model": "Qwen3-14B",
